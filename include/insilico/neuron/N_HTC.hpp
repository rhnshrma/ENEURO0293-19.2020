...
#include "current/I_Leak_HTC.hpp"
#include "current/I_T_TC.hpp"
#include "current/I_THT_HTC.hpp"
#include "current/I_AHP_HTC.hpp"
#include "current/I_H_HTC.hpp"

namespace insilico {

class N_HTC: public Neuron{
 public:
  void ode_set(state_type &variables, state_type &dxdt, const double t,const unsigned index) {
    ...
    for(std::vector<unsigned>::size_type iterator = 0; iterator < v_pre.size(); ++iterator) {
      I_Syn = I_Syn + 0.004*(v-v_pre.at(iterator)); 
    }
