...
class N_TC_mGluR : public Neuron{
 public:
  void ode_set(state_type &variables, state_type &dxdt, const double t,const unsigned index) {
    ...
    if((t-T_next_epsp) > 0)
    {
     ...
     if((t-T_next_ipsp) > 0)
    {
        double zz = random2::rand<double>();
        double next_T_ipsp = -1*(log(1-zz)*10);
    ...
    double I_ipsp = ((t < (T_last_ipsp + 2)) ? (1.5*gs*exp(T_last_ipsp - t)*(v + 85)):0 );
    ...
    // ODE
    dxdt[v_index] =  I_Na + I_K + I_H + I_TC_T + I_Leak + I_Ext - I_Syn - I_epsp -I_ipsp;
  }
};
