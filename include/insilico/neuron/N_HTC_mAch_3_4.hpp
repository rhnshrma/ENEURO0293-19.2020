...
class N_HTC_mAch_1: public Neuron{
 public:
  void ode_set(state_type &variables, state_type &dxdt, const double t,const unsigned index) {
    ...
    unsigned v_pre_1 = engine::neuron_index(1,"v");
    unsigned v_pre_2 = engine::neuron_index(2,"v");
    unsigned v_pre_3 = engine::neuron_index(3,"v");
    ...
    for(std::vector<unsigned>::size_type iterator = 0; iterator < g1_indices.size(); ++iterator) {
      ...
      I_Syn_GJ = engine::neuron_value(index,"g_gj")*((v-variables[v_pre_1])+(v-variables[v_pre_2])+(v-variables[v_pre_3])); 
